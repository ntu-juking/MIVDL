...
def train(model, dataset, max_steps, dev_every, loss_function, optimizer, save_path, log_every=50, max_patience=5):
    ...
    torch.save(model.state_dict(), _save_file)
    _save_file.close()
    acc, pr, rc, f1,auc= evaluate_metrics(model, loss_function, dataset.initialize_train_batch(),
                                       dataset.get_next_train_batch)
    debug('%s\tTest Accuracy: %0.2f\tPrecision: %0.2f\tRecall: %0.2f\tF1: %0.2f\tAUC: %0.2f' % (save_path, acc, pr, rc, f1,auc))
    debug('=' * 100)
